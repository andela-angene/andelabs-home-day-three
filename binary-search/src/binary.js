...
Array.prototype.search = function search(n) {
    ...
    var length  = this.length,
        ...
        while (this[mid - 1] !== n) {
            ...
            if (low === mid && this[mid - 1] !== n) return {length: length, index: -1, count: count};
        }
        if (n === this[length - 1]) return {length: length, index: (high - 1), count: 0}
        return {length: length, index: (mid - 1), count: count};
}
